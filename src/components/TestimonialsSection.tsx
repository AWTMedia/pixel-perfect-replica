      ...
      <div className="container mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {testimonials.map((testimonial, index) => (
            <div
              key={index}
              className="bg-secondary/70 border border-border rounded-2xl p-6 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all"
            >
          ...
          {/* Purple feature card */}
          <div className="bg-primary rounded-2xl p-6 flex flex-col justify-center shadow-md">
            <h3 className="text-white font-bold text-xl mb-2">{ratingInfo.title}</h3>
            <p className="text-white/80 text-sm leading-relaxed">{ratingInfo.description}</p>
          </div>
        </div>
      </div>
    </section>
